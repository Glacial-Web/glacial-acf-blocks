...
Dics.prototype._load = function (firstImage) {
  ...
  var maxCounter = 1 < arguments.length && arguments[1] !== undefined ? arguments[1] : 100000;
...
Dics.prototype._applyGlobalClass = function (options) {
  ...
  if ('vertical' === options.linesOrientation) {
    container.classList.add('b-dics--vertical');
  }

  if ('center' === options.textPosition) {
    container.classList.add('b-dics--tp-center');
  } else if ('bottom' === options.textPosition) {
    container.classList.add('b-dics--tp-bottom');
  } else if ('left' === options.textPosition) {
    container.classList.add('b-dics--tp-left');
  } else if ('right' === options.textPosition) {
...
Dics.prototype._pushSections = function (calcMovePixels, position) {
  ...
  section.style.flex = true === this._isGoingRight ? '2 0 '.concat(calcMovePixels, 'px') : '1 1 '.concat(calcMovePixels, 'px');
  postActualSection.style.flex = true === this._isGoingRight ? ' '.concat(sectionWidth, 'px') : '2 0 '.concat(sectionWidth, 'px');
    ...
    function recursiveMerge(target, object) {
      for (var property in object) {
        if (object.hasOwnProperty(property)) {
          ...
          if ('Object' === utils.getConstructor(current)) {
            if (!target[property]) {
              target[property] = {};
            }
